\begin{figure}[!ht]
	...
	(tsinv.up) --++ (0,0.5) node[anchor=west, scale=1]{ctrl} |-++ (-3.5,0.2) node[ieeestd not port, anchor=out, scale=1](not) {} 
	(tsinv.down) --++ (0,-0.4) node[anchor=west, scale=1]{$\overline{\mbox{ctrl}}$}
	(not.in) --++ (-1,0) node[ieeestd nand port, anchor=out, scale=1](nor){}
        (nor.in 2) -|++ (0, -0.6) node[ieeestd nor port, anchor=out, scale=1](and){}
	...
	\draw (csin) ++ (2.5,2.5) node[anchor=south west](andorlabel){\small НИЛИ-И}; 
	\draw (csin) ++ (7,2.1) node[anchor=south west](tsinvlabel){\small Тростатички инвертор}; 
	%\draw[dashed] (not.out) --++ (0,0.9);
	\draw[thick] (tsinv.up) node[draw, circle, anchor=south, fill=white, minimum size=0.17cm, inner sep=0] (invup) {};
        %\draw[decoration={brace, mirror},decorate] (not.out) ++ (0,0.9) --++ (-3.25,0) node[above=2pt] {\small И-ИЛИ} --++ (-3.23,0);
    \end{tikzpicture}
    \label{DCO_cell_active_low_logic}
    }
    \vfil
    %\hfil
    \vspace{0.5cm}
    ...
    %\vspace{0.2cm}
    \caption{Шема ћелије \DCO-а на нивоу (а) логичких кола и (б) транзистора за управљачке сигнале активне на низак логички ниво.}
    \label{DCO_cell_active_low}
\end{figure}
